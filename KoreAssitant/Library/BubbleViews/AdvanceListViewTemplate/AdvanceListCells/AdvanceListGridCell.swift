...
class AdvanceListGridCell: UITableViewCell {
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        // Initialization code
        collectionV.register(UINib.init(nibName: "AdvanceListCollectionViewCell", bundle: bundle), forCellWithReuseIdentifier: "AdvanceListCollectionViewCell")
    }
        ...
        if let iconImg = details.icon{
            if iconImg.contains("base64"){
                ...
                if let url = URL(string: iconImg){
                    cell.imagV.setImageWith(url, placeholderImage: UIImage.init(named: "placeholder_image", in: bundle, compatibleWith: nil))
                }
            }
