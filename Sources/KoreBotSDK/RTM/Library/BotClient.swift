...
open class BotClient: NSObject, RTMPersistentConnectionDelegate {
   ...
   open var connectionDidClose: ((UInt16?, String?) -> Void)?
    ...
    open func subscribeToNotifications(_ deviceToken: String!, success:((Bool) -> Void)?, failure:((_ error: Error?) -> Void)?) {
        let requestManager: HTTPRequestManager = HTTPRequestManager.sharedManager
        requestManager.subscribeToNotifications(deviceToken, userInfo: userInfoModel, authInfo: authInfoModel, success: success, failure: failure)
    }
    
    open func unsubscribeToNotifications(_ deviceToken: String!, success:((Bool) -> Void)?, failure:((_ error: Error?) -> Void)?) {
        let requestManager: HTTPRequestManager = HTTPRequestManager.sharedManager
        requestManager.unsubscribeToNotifications(deviceToken, userInfo: userInfoModel, authInfo: authInfoModel, success: success, failure: failure)
    }
