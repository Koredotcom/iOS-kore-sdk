...
//        let clientSecret: String = SDKConfiguration.botConfig.clientSecret
        ...
        let xAuth:String?
        let authorizationXAuthInfo: [String: Any]?
        
        if authorizationToken != nil || xAuthToken != nil{
             accessToken = authorizationToken
             xAuth = xAuthToken
             authorizationXAuthInfo = ["Authorization":accessToken! as String, "XAuth": xAuth! as String]
        }else{
             accessToken = ""
             authorizationXAuthInfo = ["sessionId":"23134"]
        }
    ...
    func brandingApiRequest(_ accessToken: String!, success:((_ brandingArray: NSArray) -> Void)?, failure:((_ error: Error) -> Void)?) {
        ...
        // NOTE: You must set your URL to generate JWT.
        //https://wb.korebots.com/workbench/api/workbench/sdkData?objectId=hamburgermenu&objectId=brandingwidgetdesktop
