...
class NewListBubbleView: BubbleView {
    ...
    override func populateComponents() {
        if (components.count > 0) {
            ...
            if (component.componentDesc != nil) {
                ...
                do{
                     let jsonData = try DictionaryDecoder().decode(Payload.self, from: jsonObject as! [String : Any])
                    self.arrayOfElements = jsonData.elements
                    self.arrayOfButtons = jsonData.buttons
                    self.titleLbl.text = jsonData.text
                    
                    //jsonDic = try DictionaryDecoder().decode(Payload.self, from: jsonObject as! [String : Any])
                    //print("self.arrayOfElements....\(jsonDic?.elements)")
    ...
    override var intrinsicContentSize : CGSize {
        ...
        if arrayOfElements.count > rowsDataLimit{
...
extension NewListBubbleView: UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if elements.defaultAction.fallbackURL != nil{
             self.linkAction(elements.defaultAction.fallbackURL)
        }else{
            self.optionsAction(elements.defaultAction.title)
        }
    }
    ...
    func tableView(_ tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat {
        return arrayOfElements.count > rowsDataLimit ? 30 : 0
    }
