...
class ChatMessagesViewController: UIViewController, BotMessagesViewDelegate, ComposeBarViewDelegate, KREGrowingTextViewDelegate, NewListViewDelegate, TaskMenuNewDelegate, calenderSelectDelegate, ListWidgetViewDelegate, feedbackViewDelegate {
    ...
    func setUpNavigationBar(){
        let urlString = brandingShared.brandingInfoModel?.bankLogo?.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
         ...
         var image = UIImage(named: "mashraqLogo", in: frameworkBundle, compatibleWith: nil)
         ...
         let font:UIFont? = UIFont(name: "Gilroy-Regular", size:16)
    ...
    func configureQuickReplyView() {
        ...
        self.quickSelectContainerView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-15-[quickReplyView]|", options:[], metrics:nil, views:["quickReplyView" : self.quickReplyView]))
...
//                    self.botMessagesView.tableView.reloadRows(at: [indexPosition], with: .none)
//                  }
//                }
//                isReloadTabV = false
            })
    ...
    func fetchMessages() {
        ...
        dataStoreManager.getMessagesCount(completion: { [weak self] (count) in
            if count == 0 {
                ...
                self?.offSet =  count + welcomeMsgRemoveCount
