...
open class RTMPersistentConnection : NSObject, WebSocketDelegate {
    ...
    open func start() {
        ...
        let reWriteSocketUrl = SDKConfiguration.serverConfig.reWriteSocketUrl
        if reWriteSocketUrl != ""{
            var ch = Character(":")
            var result = urlString.split(separator: ch, maxSplits: 2)
            let finalUrlStr = "wss://\(reWriteSocketUrl):\(result.last ?? "")"
            urlComponents = URLComponents(string: finalUrlStr)
        }
