    ...
    override func populateComponents() {
        if (components.count > 0) {
            ...
            if ((component.componentDesc) != nil) {
                ...
                let newString = parsedString.replacingOccurrences(of: ":)", with: "\u{1f642}")
                if isHtmlString(for: string) || newString.contains("*"){
                    self.textLabel.setHTMLString(newString, withWidth: kMaxTextWidth)
                }else{
                    self.textLabel.setString(newString, withWidth: kMaxTextWidth)
                }
