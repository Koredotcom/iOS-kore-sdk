...
open class KREOptionsView: UIView, UITableViewDataSource, UITableViewDelegate {
    ...
    public var buttonRowHeight: CGFloat = 44 {
        didSet {
            optionsTableView.rowHeight = buttonRowHeight
            optionsTableView.estimatedRowHeight = buttonRowHeight
            optionsTableView.reloadData()
        }
    }
    var kMaxRowHeight: CGFloat { buttonRowHeight }
    var buttonHeight: CGFloat { max(buttonRowHeight - buttonSpacing, 0) }
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(option.optionType == KREOptionType.button){
            ...
            if let cell = cell as? KREOptionsTableViewNewCell {
                ...
                cell.titleLabel?.font = UIFont(name: mediumCustomFont, size: 16.0)
                cell.applyButtonHeight(buttonHeight, spacing: buttonSpacing)
            }
