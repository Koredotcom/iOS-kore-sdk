...
public class KRETokenCollectionViewCell: UICollectionViewCell {
    ...
    var krefocused: Bool = false {
        didSet {
            self.label.font = UIFont(name: fontName, size: 16.0)
            // Default + selected use user-bubble colors for quick_replies.
            self.backgroundColor = BubbleViewRightTint
            self.layer.borderColor = BubbleViewRightTint.cgColor
            self.label.textColor = BubbleViewUserChatTextColor
    ...
    func setup() {
        ...
        self.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-6-[image]-(10)-[label]-(16)-|", options:[], metrics:nil, views:views))
    ...
    func widthForCell(string: String?, withImage: Bool, height: CGFloat, fontName: String) -> CGFloat {
        ...
        return width + 24.0 + (withImage ? 32.0 : 0.0) + 8.0
    }
}
