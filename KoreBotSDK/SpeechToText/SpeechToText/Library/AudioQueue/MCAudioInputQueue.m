...
{
    //    OSStatus status =  AudioQueueNewInput(&_format, MCAudioQueueInuputCallback, (__bridge void *)(self), CFRunLoopGetCurrent(),
    //                                          kCFRunLoopCommonModes, 0, &_audioQueue);
    if (![self _checkAudioQueueSuccess:AudioQueueNewInput(&_format, MCAudioQueueInuputCallback, (__bridge void *)(self), CFRunLoopGetCurrent(),
                                                          kCFRunLoopCommonModes, 0, &_audioQueue)])
    {
        return;
    }
...
- (BOOL)stop
{
    ...
    NSLog(@"Stop audioqueue recording called");
    return status == noErr;
}
                         ...
                         inPacketDescs:(const AudioStreamPacketDescription *)inPacketDescs
{
    if (_started)
    {
        ...
        [_delegate inputQueue:self inputData:data numberOfPackets:inNumberPacketDescriptions];
...
- (void)handleAudioQueuePropertyCallBack:(AudioQueueRef)audioQueue property:(AudioQueuePropertyID)property
{
    if (property == kAudioQueueProperty_IsRunning)
    {
        ...
        if(!_isRunning){
            [_delegate inputQueue:self inputData:_buffer numberOfPackets:1];
        }
    }
}
