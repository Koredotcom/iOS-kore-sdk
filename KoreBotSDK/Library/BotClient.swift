...
open class BotClient: NSObject, RTMPersistentConnectionDelegate {
    ...
    fileprivate func rtmConnectionWithBotInfoModel(_ botInfo: BotInfoModel, isReconnect: Bool) -> RTMPersistentConnection? {
        if let connection = connection, (connection.websocket?.readyState == .OPEN || connection.websocket?.readyState == .CONNECTING) {
            ...
            let dic: [String:Any] = ["scheme": "wss", "host": "searchassist-qa.kore.ai"] //kk dev.findly.ai qa.findly.ai
            reWriteOptions = dic 
    ...
    open func sendMessage(_ message: String, dictionary: [String: Any]? = nil, options: [String: Any]?, taskMetaData: [String:Any]?, botInfoCustomData: [String:Any]? ) {
        ...
        case .OPEN:
            ...
            connection.sendMessage(message, parameters: parameters, options: options, taskMetaData: taskMetaData, botInfoCustomData: botInfoCustomData)
            break
        case .CLOSED:
            break
        case .CLOSING:
