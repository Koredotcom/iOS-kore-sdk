...
class TextBubbleView : BubbleView {
    ...
    private var messageRegularFont: UIFont {
        return UIFont.textFont(ofSize: 16.0, weight: .regular)
    }
    ...
    override func initialize() {
        ...
        self.textLabel.font = UIFont(name: regularCustomFont, size: 16.0)
        ...
        if (components.count > 0) {
            ...
            if ((component.componentDesc) != nil) {
                ...
                if let parsedString = KREUtilities.formatHTMLEscapedString(htmlStrippedString) {
                    ...
                    //applyRegularMessageFont(for: replaceStr)
                }else{
                    var replaceStr = string
                    if isEmojiDispaly{
                        replaceStr = replaceUnicodeWithEmojis(input: replaceStr)
                    }
                    //self.textLabel.font = messageRegularFont
    ...
    func updateStreamingText(_ text: String) {
        ...
        if let parsedString = KREUtilities.formatHTMLEscapedString(htmlStrippedString) {
            ...
            replaceStr = replaceStr.replacingOccurrences(of: "&quot;", with: "\"")
            //textLabel.font = messageRegularFont
    ...
    fileprivate func applyRegularMessageFont(for sourceText: String) {
        guard let attributedText = textLabel.attributedText,
              attributedText.length > 0 else {
            return
        }

        let regularFont = messageRegularFont
        let mutableText = NSMutableAttributedString(attributedString: attributedText)
        let fullRange = NSRange(location: 0, length: mutableText.length)
        let containsExplicitBoldMarkdown = sourceText.contains("**")
            || sourceText.contains("__")

        mutableText.enumerateAttribute(.font, in: fullRange) { value, range, _ in
            guard let font = value as? UIFont else {
                mutableText.addAttribute(.font, value: regularFont, range: range)
                return
            }

            let traits = font.fontDescriptor.symbolicTraits
            if containsExplicitBoldMarkdown && traits.contains(.traitBold) {
                return
            }
            ...
            if (component.componentDesc != nil) {
                ...
                if let string: String = jsonObject["text"] as? String {
                    ...
                    if let parsedString = KREUtilities.formatHTMLEscapedString(htmlStrippedString) {
                        ...
                        //applyRegularMessageFont(for: parsedString)
                    }
                }else{
