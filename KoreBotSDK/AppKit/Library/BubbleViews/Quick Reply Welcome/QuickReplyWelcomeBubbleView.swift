...
class QuickReplyWelcomeBubbleView: BubbleView {
    ...
    private var titleChipConstraints: [NSLayoutConstraint] = []
    ...
    override func initialize() {
        ...
        self.titleLbl.font = UIFont(name: regularCustomFont, size: 16.0)
    ...
    /// Re-applies LTR/RTL chrome after `refreshChatLanguageDirection` / table reload.
    func applyLanguageDirection() {
        ...
        cardView?.semanticContentAttribute = semanticDirection
        tileBgv?.semanticContentAttribute = semanticDirection
        collectionView?.semanticContentAttribute = semanticDirection
        titleLbl?.textAlignment = isKoreSDKRTL ? .right : .left

        if tileBgv != nil, cardView != nil {
            NSLayoutConstraint.deactivate(titleChipConstraints)
            cardView.removeConstraints(titleChipConstraints)
            let titleChipFormat = isKoreSDKRTL ? "H:[tileBgv]-0-|" : "H:|-0-[tileBgv]"
            titleChipConstraints = NSLayoutConstraint.constraints(
                withVisualFormat: titleChipFormat,
                options: [.directionLeftToRight],
                metrics: nil,
                views: ["tileBgv": tileBgv as Any]
            )
            cardView.addConstraints(titleChipConstraints)
        }
    ...
    func setCornerRadiousToTitleView(){
        ...
        if #available(iOS 11.0, *) {
            if bubbleStyle == "balloon"{
                if isKoreSDKRTL{
                    self.tileBgv.roundCorners([.layerMaxXMinYCorner,.layerMinXMinYCorner, .layerMinXMaxYCorner], radius: radius, borderColor: borderColor, borderWidth: borderWidth)
                }else{
                    self.tileBgv.roundCorners([.layerMaxXMinYCorner,.layerMinXMinYCorner, .layerMaxXMaxYCorner], radius: radius, borderColor: borderColor, borderWidth: borderWidth)
                }
        ...
        if (components.count > 0) {
            ...
            if (component.componentDesc != nil) {
                if ((component.componentDesc) != nil) {
                    ...
                    if jsonObject["text"] is String{
                        self.titleLbl.text = newString
                    }else{
                        self.titleLbl.text = ""
                    }
    ...
    override var intrinsicContentSize : CGSize {
        ...
        if textSize.width == 0.0{
            ...
            self.collectionVTopConstraint.isActive = false
            let collectionviewHeight  = Double(self.collectionView.collectionViewLayout.collectionViewContentSize.height)
            return CGSize(width: 0.0, height: textSize.height + 35 + CGFloat(collectionviewHeight))
        }
    }
...
extension QuickReplyWelcomeBubbleView : UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if seletedRow == indexPath.row {
            ...
            cell.bgV.backgroundColor = BubbleViewRightTint
            cell.textlabel.textColor = BubbleViewUserChatTextColor
            cell.layer.borderColor =  BubbleViewRightTint.cgColor
        }
