...
var koreSDKLocale = Locale(identifier: "en")

var arrayOfViews = [BubbleView.Type]()
var arrayOfTemplateTypes = [String]()
var quickRepliesIsHorizontal = true
...
open class Utilities: NSObject {
    ...
    public static func getTimeformater(sentOn: Date) -> String{
        var datePattern = brandingBodyDic.time_stamp?.date_format ?? "dd/MM/yyyy"
        if datePattern.lowercased() == "dd/mm/yyyy" {
            datePattern = "dd/MM/yyyy"
        } else if datePattern.lowercased() == "mm/dd/yyyy" {
            datePattern = "MM/dd/yyyy"
        } else if datePattern.lowercased() == "mmm/dd/yyyy" {
            datePattern = "MMM/dd/yyyy"
        }
        datePattern = datePattern.replacingOccurrences(of: "D", with: "d")
        datePattern = datePattern.replacingOccurrences(of: "Y", with: "y")

        let uses24HourTime = brandingBodyDic.time_stamp?.timeformat == "24"
        let timePattern = uses24HourTime ? "HH:mm:ss" : "h:mm:ss"
        let formatter = DateFormatter()
        formatter.locale = Locale(identifier: "en_US_POSIX")
        formatter.dateFormat = "\(datePattern) \(timePattern)"
        var formattedTimeStamp = formatter.string(from: sentOn)

        if !uses24HourTime {
            let hour = Calendar.current.component(.hour, from: sentOn)
            let isArabic = koreSDKLocale.languageCode == "ar"
            let dayPeriod = isArabic
                ? (hour < 12 ? "ص" : "م")
                : (hour < 12 ? "AM" : "PM")
            formattedTimeStamp += " \(dayPeriod)"
        }
        return formattedTimeStamp
    }
    
    public static func getDateformater(sentOn: Date) -> String{
        if Calendar.current.isDateInToday(sentOn) {
            return getConversationDayHeader(sentOn: sentOn)
        }
        ...
        dateFormt.locale = koreSDKLocale
        ...
        if dateFormt.string(from: sentOn) == dateFormt.string(from: today){
            ...
            dateFormatter.locale = koreSDKLocale
            ...
            timeFormatter.locale = koreSDKLocale
        ...
        if is12or24Format{
            ...
            if let time_format = brandingBodyDic.time_stamp?.timeformat, time_format == "24"{
                ...
                timeFormatter24.locale = koreSDKLocale
                ...
                timeFormatter12.locale = koreSDKLocale
    ...
    private static func getConversationDayHeader(sentOn: Date) -> String {
        let calendar = Calendar.current
        let day = calendar.component(.day, from: sentOn)
        ...
        return "\(monthFormatter.string(from: sentOn)) "
            + "\(day)\(ordinalSuffix(for: day)) "
            + timeFormatter.string(from: sentOn)
    }

    private static func ordinalSuffix(for day: Int) -> String {
        let lastTwoDigits = day % 100
        if 11...13 ~= lastTwoDigits {
            return "th"
        }
