...
open class BotConnect: NSObject {
    ...
    func customSettings(){
        let normalizedLanguage = koreSDkLanguage.replacingOccurrences(of: "_", with: "-")
        let languageCode = normalizedLanguage
            .split(separator: "-")
            .first
            .map(String.init) ?? koreSDkLanguage
        koreSDKLocale = Locale(identifier: normalizedLanguage.replacingOccurrences(of: "-", with: "_"))
    ...
    func getLaguageValues(bundle: Bundle){
        onlineStatus = bundle.localizedString(forKey: "onlineStatus", value: "Online", table: nil)
