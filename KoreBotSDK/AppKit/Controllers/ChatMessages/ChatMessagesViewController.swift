...
public class ChatMessagesViewController: UIViewController, BotMessagesViewDelegate, ComposeBarViewDelegate, KREGrowingTextViewDelegate, NewListViewDelegate, TaskMenuNewDelegate, calenderSelectDelegate, ListWidgetViewDelegate, feedbackViewDelegate, CustomTableTemplateDelegate, WelcomeScreenViewDelegate, ArticleViewDelegate, OTPValidationDelegate, resetPinDelegate, AdvancedMultiSelectDelegate {
    ...
    private weak var closeOrMinimizeMessageLabel: UILabel?
    private weak var closeOrMinimizeCancelButton: UIButton?
    private weak var closeOrMinimizeCloseButton: UIButton?
    private weak var closeOrMinimizeMinimizeButton: UIButton?
...
//                        }
    ...
    func receviceMessage(dictionary:[String: Any]){
        if let botLanguage = dictionary["botLanguage"] as? String {
            botLanguageDidChange(botLanguage)
        }
        ...
        for i in 0..<data.count{
            ...
            if let botLanguage = valData["botLanguage"] as? String {
                botLanguageDidChange(botLanguage)
            }
            var textMessage: Message! = nil
    ...
    func populateQuickReplyCards(with message: KREMessage?) {
        if message?.templateType == (ComponentType.quickReply.rawValue as NSNumber) {
            ...
            if ((component.componentDesc) != nil) {
                ...
                self.quickReplyView.bgColor = ""
                self.quickReplyView.textColor = ""
                self.quickReplyView.boarderColor = ""
                self.quickReplyView.fontName = mediumCustomFont
    ...
    @objc func startSpeaking(notification:Notification) {
        if SDKConfiguration.botConfig.showTextToSpeech && isSpeakingEnabled {
...
extension ChatMessagesViewController: KABotClientDelegate {
    public func botLanguageDidChange(_ language: String) {
        let normalizedLanguage = language
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        guard normalizedLanguage == "en" || normalizedLanguage == "ar" else {
            return
        }
        guard koreSDKLocale.languageCode?.lowercased() != normalizedLanguage else {
            return
        }

        koreSDKLocale = Locale(identifier: normalizedLanguage)
        isKoreSDKRTL = normalizedLanguage == "ar"
        preferredLanguageChangeHandler?(normalizedLanguage)
        refreshChatLanguageDirection()
    }

    private func refreshChatLanguageDirection() {
        ...
        threadContainerView.semanticContentAttribute = semanticDirection
        composeBarContainerView.semanticContentAttribute = semanticDirection
        quickSelectContainerView.semanticContentAttribute = semanticDirection
        typingStatusView?.semanticContentAttribute = semanticDirection

        headerView.subviews
            .filter { $0 is ChatVCHeaderView }
            .forEach { $0.removeFromSuperview() }
        if let brandedHeader = brandingValues.header {
            configureHeaderView(headerDic: brandedHeader)
        }
        headerView.bringSubviewToFront(linerProgressBar)

        if composeView != nil {
            let draftText = composeView.growingTextView.textView.text ?? ""
            let keyboardEnabled = composeView.isKeyboardEnabled
            let wasFirstResponder = composeView.growingTextView.textView.isFirstResponder
            composeViewBottomConstraint?.isActive = false
            composeBarContainerHeightConstraint?.isActive = false
            composeView.removeFromSuperview()
            configureComposeBar()
            composeView.setText(draftText)
            configureViewForKeyboard(keyboardEnabled)
            if wasFirstResponder {
                composeView.growingTextView.textView.becomeFirstResponder()
            }
        }

        botMessagesView?.semanticContentAttribute = semanticDirection
        botMessagesView?.tableView.semanticContentAttribute = semanticDirection
        botMessagesView?.tableView.reloadData()
        refreshCloseOrMinimizePopupLocalization()
        view.setNeedsLayout()
        view.layoutIfNeeded()
    }
    ...
    @objc func startTypingStatusForBot() {
        ...
        self.typingStatusView?.startTypingStatus(using: urlString,dotColor: BubbleViewRightTint)
    }
...
extension ChatMessagesViewController{
    ...
    func showLoader(){
        let show = { [weak self] in
            ...
            loadingLabel.text = koreSDKLocale.languageCode?.lowercased() == "ar"
                ? "يرجى الانتظار…"
                : "Please wait…"
            loadingLabel.textColor = startupLoadingColor
...
//        }
        ...
        if let bodyBubbleStyle = brandingValues.body?.bubble_style,
           !bodyBubbleStyle.isEmpty {
            brandingBodyDic.bubble_style = bodyBubbleStyle
        } else if let chatBubbleStyle = brandingValues.chat_bubble?.style,
                  !chatBubbleStyle.isEmpty {
            brandingBodyDic.bubble_style = chatBubbleStyle
        }
    ...
    func configureHeaderView(headerDic: HeaderModel){
        ...
        if size == "compact"{
            headerVHeight = 91.18181610107422
    ...
    func setupCloseOrMinimizePopup() {
        ...
        closeOrMinimizeMessageLabel = messageLabel
        ...
        closeButton.setTitleColor(BubbleViewRightTint, for: .normal)
        ...
        closeOrMinimizeCloseButton = closeButton
        ...
        minimizeButton.setTitleColor(BubbleViewRightTint, for: .normal)
        ...
        closeOrMinimizeMinimizeButton = minimizeButton
    ...
    private func refreshCloseOrMinimizePopupLocalization() {
        let semanticDirection: UISemanticContentAttribute =
            isKoreSDKRTL ? .forceRightToLeft : .forceLeftToRight
        closeOrMinimizePopupCardView?.semanticContentAttribute = semanticDirection
        closeOrMinimizeMessageLabel?.semanticContentAttribute = semanticDirection
        closeOrMinimizeMessageLabel?.textAlignment = isKoreSDKRTL ? .right : .left
        closeOrMinimizeMessageLabel?.text = closeOrMinimizeMsg
        closeOrMinimizeCancelButton?.setTitle(cancelBtnTitle, for: .normal)
        closeOrMinimizeCloseButton?.setTitle(closeBtnTitle, for: .normal)
        closeOrMinimizeMinimizeButton?.setTitle(minimizeBtnTitle, for: .normal)
        closeOrMinimizeCancelButton?.setTitleColor(BubbleViewRightTint, for: .normal)
        closeOrMinimizeCloseButton?.setTitleColor(BubbleViewRightTint, for: .normal)
        closeOrMinimizeMinimizeButton?.setTitleColor(BubbleViewRightTint, for: .normal)
    }
