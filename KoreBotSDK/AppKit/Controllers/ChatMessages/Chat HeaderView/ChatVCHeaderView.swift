...
class ChatVCHeaderView: UIView {
   ...
   //RegulartHeaderSupportBtn: UIButton!
    ...
    //RegulartHeaderCloseBtn
        ...
        if size == "compact"{
            self.headerViewHeightConstraint.constant = 91.18181610107422 //headerHeight
            ...
            configureOnlineStatus(
                replacing: compactHeaderDescLbl,
                alignment: isKoreSDKRTL ? .right : .left
            )
            ...
            configureOnlineStatus(replacing: RegulartHeaderDescLbl, alignment: .center)
    ...
    private func hideBackNavigationButtons() {
        [
            compactHeaderBackBtn,
            RegulartHeaderBackBtn,
            largeHeaderBackBtn
        ].forEach { button in
    ...
    private enum OnlineStatusAlignment {
        case left
        case center
        case right
    }
    ...
    private func configureOnlineStatus(
        replacing sourceLabel: UILabel,
        alignment: OnlineStatusAlignment
    ) {
        sourceLabel.text = nil

        let statusPill = UIView()
        statusPill.translatesAutoresizingMaskIntoConstraints = false
        statusPill.backgroundColor = UIColor(hexString: "#ECFDF3")
        statusPill.layer.cornerRadius = 10
        statusPill.layer.masksToBounds = true
        statusPill.accessibilityLabel = onlineStatus

        let statusTextLabel = UILabel()
        statusTextLabel.font = UIFont(name: regularCustomFont, size: 11.0)
            ?? UIFont.systemFont(ofSize: 11.0)
        statusTextLabel.text = onlineStatus
        statusTextLabel.textColor = UIColor(hexString: "#176B4A")

        let statusDotLabel = UILabel()
        statusDotLabel.text = "●"
        statusDotLabel.textColor = UIColor(hexString: "#15825B")
        statusDotLabel.font = UIFont.systemFont(ofSize: 9.0)

        let arrangedSubviews = isKoreSDKRTL
            ? [statusTextLabel, statusDotLabel]
            : [statusDotLabel, statusTextLabel]
        let statusStack = UIStackView(arrangedSubviews: arrangedSubviews)
        statusStack.translatesAutoresizingMaskIntoConstraints = false
        statusStack.axis = .horizontal
        statusStack.alignment = .center
        statusStack.spacing = 6
        statusStack.semanticContentAttribute = .forceLeftToRight
        statusPill.addSubview(statusStack)

        guard let container = sourceLabel.superview else {
            return
        }
        container.addSubview(statusPill)

        var constraints = [
            statusPill.centerYAnchor.constraint(equalTo: sourceLabel.centerYAnchor),
            statusPill.heightAnchor.constraint(equalToConstant: 20),
            statusStack.leadingAnchor.constraint(equalTo: statusPill.leadingAnchor, constant: 8),
            statusStack.trailingAnchor.constraint(equalTo: statusPill.trailingAnchor, constant: -8),
            statusStack.centerYAnchor.constraint(equalTo: statusPill.centerYAnchor)
        ]
        switch alignment {
        case .left:
            constraints.append(statusPill.leftAnchor.constraint(equalTo: sourceLabel.leftAnchor))
        case .center:
            constraints.append(statusPill.centerXAnchor.constraint(equalTo: sourceLabel.centerXAnchor))
        case .right:
            constraints.append(statusPill.rightAnchor.constraint(equalTo: sourceLabel.rightAnchor))
        }
