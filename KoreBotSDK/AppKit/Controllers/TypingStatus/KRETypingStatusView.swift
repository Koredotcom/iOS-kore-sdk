...
open class KRETypingStatusView: UIView {
    ...
    private var typingHorizontalConstraints = [NSLayoutConstraint]()
    ...
    private func updateDotsMargin() {
        NSLayoutConstraint.deactivate(typingHorizontalConstraints)
        let leadingMargin: CGFloat = isKoreSDKRTL ? 13.0 : 8.0
        let views: [String: Any] = ["typingImageView": typingImageView, "dancingDots": dancingDots]
        typingHorizontalConstraints = NSLayoutConstraint.constraints(
            withVisualFormat: "H:|-(leadingMargin)-[typingImageView(0)]-2-[dancingDots(30)]-|",
            options: [],
            metrics: ["leadingMargin": leadingMargin],
            views: views
        )
        NSLayoutConstraint.activate(typingHorizontalConstraints)
    }
